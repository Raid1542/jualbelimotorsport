<nav class="bg-white shadow-md sticky top-0 z-50 transition duration-300">
  <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
    ...
    <div class="hidden lg:flex flex-1 justify-center px-6">
      <form action="{{ route('produk') }}" method="GET" class="relative w-full max-w-md">
        <input type="text" name="keyword" value="{{ request('keyword') }}" placeholder="Cari produk..."
          class="w-full border border-gray-300 rounded-full py-2 pl-4 pr-10 text-sm text-gray-700 shadow-inner
    ...
    <div class="hidden lg:flex items-center space-x-5">
      {{-- Menu --}}
      <a href="{{ route('produk', ['from' => 'dashboard']) }}" class="text-gray-700 hover:text-yellow-600 font-semibold transition duration-300 px-3 py-2 rounded-md">Produk</a>
      <a href="{{ route('tentang') }}" class="text-gray-700 hover:text-yellow-600 font-semibold transition duration-300 px-3 py-2 rounded-md">Tentang</a>
      <a href="{{ route('pesanan') }}" class="text-gray-700 hover:text-yellow-600 font-semibold transition duration-300 px-3 py-2 rounded-md">Pesanan</a>
      ...
      <div class="relative group">
        ...
        <div class="absolute right-0 mt-2 w-80 bg-white border rounded-xl shadow-xl z-50 
          ...
          <div class="max-h-80 overflow-y-auto divide-y">
            @forelse($keranjangDropdown->take(5) as $item)
              <div class="flex items-center gap-4 px-4 py-3 hover:bg-gray-50 transition">
                <div class="w-14 h-14 flex-shrink-0 border rounded overflow-hidden bg-white shadow">
          ...
          {{-- Footer jika item > 5 --}}
          @if($keranjangDropdown->count() > 5)
          <div class="text-center p-3 bg-gray-50 border-t">
            <a href="{{ route('keranjang.index') }}" class="text-yellow-600 hover:underline text-sm font-medium">
              Tampilkan semua ({{ $keranjangDropdown->count() }})
            </a>
          </div>
          @endif
        </div>
